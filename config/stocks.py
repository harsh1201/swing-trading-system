"""
config/stocks.py — NSE stock universe (~420 liquid names).

Coverage  (approximate Nifty 500 composition):
  • Nifty 50             ~50  large-caps / index heavyweights
  • Nifty Next 50        ~50  large / upper-mid caps
  • Nifty Midcap 150     ~150 liquid midcaps
  • Additional           ~170 sector leaders, liquid small-caps

Tickers use Yahoo Finance NSE format  (suffix ".NS").
Stocks with very low volume or questionable data quality are excluded.
The liquidity filter in strategies/long_breakout.py provides a further
run-time check (₹5 Cr minimum daily turnover) on top of this list.
"""

STOCKS: dict[str, str] = {
    # ── BANKING ────────────────────────────────────────────────────────────────
    "HDFCBANK.NS": "HDFC Bank",
    "ICICIBANK.NS": "ICICI Bank",
    "KOTAKBANK.NS": "Kotak Bank",
    "SBIN.NS": "SBI",
    "AXISBANK.NS": "Axis Bank",
    "INDUSINDBK.NS": "IndusInd Bank",
    "BANDHANBNK.NS": "Bandhan Bank",
    "FEDERALBNK.NS": "Federal Bank",
    "IDFCFIRSTB.NS": "IDFC First Bank",
    "RBLBANK.NS": "RBL Bank",
    "BANKBARODA.NS": "Bank of Baroda",
    "PNB.NS": "Punjab Natl Bank",
    "CANBK.NS": "Canara Bank",
    "UNIONBANK.NS": "Union Bank",
    "INDIANB.NS": "Indian Bank",
    "AUBANK.NS": "AU Small Fin. Bank",
    "EQUITASBNK.NS": "Equitas Small Fin.",
    "UJJIVANSFB.NS": "Ujjivan Small Fin.",
    "IDBI.NS": "IDBI Bank",
    "KTKBANK.NS": "Karnataka Bank",
    "DCBBANK.NS": "DCB Bank",
    "CSBBANK.NS": "CSB Bank",
    "BANKINDIA.NS": "Bank of India",
    "MAHABANK.NS": "Bank of Maharashtra",
    "CENTRALBK.NS": "Central Bank of India",
    "IOB.NS": "Indian Overseas Bank",
    "UCOBANK.NS": "UCO Bank",
    "YESBANK.NS": "Yes Bank Ltd.",
    "J&KBANK.NS": "Jammu & Kashmir Bank Ltd.",
    "CUB.NS": "City Union Bank",
    "KARURVYSYA.NS": "Karur Vysya Bank",
    "SOUTHBANK.NS": "South Indian Bank",
    
    # ── NBFC & FINANCIAL SERVICES ──────────────────────────────────────────────
    "BAJFINANCE.NS": "Bajaj Finance",
    "BAJAJFINSV.NS": "Bajaj Finserv",
    "CHOLAFIN.NS": "Cholamandalam Fin.",
    "MUTHOOTFIN.NS": "Muthoot Finance",
    "MANAPPURAM.NS": "Manappuram Fin.",
    "ABCAPITAL.NS": "Aditya Birla Cap.",
    "SBICARD.NS": "SBI Cards",
    "SHRIRAMFIN.NS": "Shriram Finance",
    "M&MFIN.NS": "M&M Finance",
    "IIFL.NS": "IIFL Finance",
    "IIFLCAPS.NS": "IIFL Capital Services Ltd.",
    "POONAWALLA.NS": "Poonawalla Fin.",
    "MASFIN.NS": "MAS Financial",
    "CREDITACC.NS": "Credit Access Grameen",
    "5PAISA.NS": "5paisa Capital",
    "360ONE.NS": "360 ONE WAM Ltd.",
    "AADHARHFC.NS": "Aadhar Housing Finance Ltd.",
    "AIIL.NS": "Authum Investment & Infrastructure Ltd.",
    "BAJAJHFL.NS": "Bajaj Housing Finance Ltd.",
    "CANFINHOME.NS": "Can Fin Homes Ltd.",
    "CGCL.NS": "Capri Global Capital Ltd.",
    "FIVESTAR.NS": "Five-Star Business Finance Ltd.",
    "HDBFS.NS": "HDB Financial Services Ltd.",
    "IFCI.NS": "IFCI Ltd.",
    "LTF.NS": "L&T Finance Ltd.",
    "LICHSGFIN.NS": "LIC Housing Finance Ltd.",
    "PIRAMALFIN.NS": "Piramal Finance Ltd.",
    "PFC.NS": "Power Finance Corporation Ltd.",
    "SBFC.NS": "SBFC Finance Ltd.",
    "SAMMAANCAP.NS": "Sammaan Capital Ltd.",
    "TATACAP.NS": "Tata Capital Ltd.",
    "SUNDARMFIN.NS": "Sundaram Finance",
    "JMFINANCIL.NS": "JM Financial",
    "MFSL.NS": "Max Financial Svcs",
    "CHOLAHLDNG.NS": "Cholamandalam Inv.",
    "RECLTD.NS": "REC Ltd",
    "IREDA.NS": "IREDA",
    "AAVAS.NS": "Aavas Financiers",
    "HOMEFIRST.NS": "Home First Finance",
    "APTUS.NS": "Aptus Value Housing",
    "PNBHOUSING.NS": "PNB Housing Finance",
    "REPCOHOME.NS": "Repco Home Finance",
    "SPANDANA.NS": "Spandana Sphoorty",
    "FUSION.NS": "Fusion Micro Finance",
    
    # ── INSURANCE ─────────────────────────────────────────────────────────────
    "LICI.NS": "LIC India",
    "SBILIFE.NS": "SBI Life",
    "HDFCLIFE.NS": "HDFC Life",
    "ICICIGI.NS": "ICICI Lombard",
    "ICICIPRULI.NS": "ICICI Pru Life",
    "STARHEALTH.NS": "Star Health",
    "JIOFIN.NS": "Jio Financial",
    "CANHLIFE.NS": "Canara HSBC Life Insurance Company Ltd.",
    "GICRE.NS": "General Insurance Corporation of India",
    "GODIGIT.NS": "Go Digit General Insurance Ltd.",
    "NIVABUPA.NS": "Niva Bupa Health Insurance Company Ltd.",
    "NIACL.NS": "The New India Assurance Company Ltd.",
    
    # ── ASSET MANAGEMENT ──────────────────────────────────────────────────────
    "HDFCAMC.NS": "HDFC AMC",
    "UTIAMC.NS": "UTI AMC",
    "NAM-INDIA.NS": "Nippon Life AMC",
    "CAMS.NS": "CAMS",
    "KFINTECH.NS": "KFin Technologies",
    "ABSLAMC.NS": "Aditya Birla Sun Life AMC Ltd.",
    "ICICIAMC.NS": "ICICI Prudential Asset Management Company Ltd.",
    
    # ── CAPITAL MARKETS ────────────────────────────────────────────────────────
    "BSE.NS": "BSE",
    "MCX.NS": "MCX",
    "CDSL.NS": "CDSL",
    "CRISIL.NS": "CRISIL",
    "ANGELONE.NS": "Angel One",
    "MOTILALOFS.NS": "Motilal Oswal FS",
    "ICRA.NS": "ICRA",
    "NUVAMA.NS": "Nuvama Wealth",
    "ANANDRATHI.NS": "Anand Rathi Wealth Ltd.",
    "GROWW.NS": "Billionbrains Garage Ventures Ltd.",
    "CHOICEIN.NS": "Choice International Ltd.",
    "IEX.NS": "Indian Energy Exchange Ltd.",
    
    # ── INFORMATION TECHNOLOGY & SERVICES ──────────────────────────────────────
    "TCS.NS": "TCS",
    "INFY.NS": "Infosys",
    "WIPRO.NS": "Wipro",
    "HCLTECH.NS": "HCL Tech",
    "TECHM.NS": "Tech Mahindra",
    "LTTS.NS": "L&T Tech Svcs",
    "PERSISTENT.NS": "Persistent Sys.",
    "COFORGE.NS": "Coforge",
    "MPHASIS.NS": "Mphasis",
    "KPITTECH.NS": "KPIT Tech",
    "TATAELXSI.NS": "Tata Elxsi",
    "OFSS.NS": "Oracle Fin. Svcs",
    "CYIENT.NS": "Cyient",
    "NAUKRI.NS": "Info Edge",
    "MASTEK.NS": "Mastek",
    "HAPPSTMNDS.NS": "Happiest Minds",
    "TANLA.NS": "Tanla Platforms",
    "NEWGEN.NS": "Newgen Software",
    "LATENTVIEW.NS": "LatentView Analytics",
    "BSOFT.NS": "Birlasoft",
    "SONATSOFTW.NS": "Sonata Software",
    "INTELLECT.NS": "Intellect Design",
    "TATATECH.NS": "Tata Technologies",
    "ZENSARTECH.NS": "Zensar Tech",
    "RATEGAIN.NS": "RateGain Travel",
    "HEXT.NS": "Hexaware Tech",
    "ECLERX.NS": "eClerx Services",
    "DATAMATICS.NS": "Datamatics Global",
    "ROUTE.NS": "Route Mobile",
    "CPPLUS.NS": "Aditya Infotech Ltd.",
    "MAPMYINDIA.NS": "C.E. Info Systems Ltd.",
    "CARTRADE.NS": "Cartrade Tech Ltd.",
    "FSL.NS": "Firstsource Solutions Ltd.",
    "INDGN.NS": "Indegene Ltd.",
    "IKS.NS": "Inventurus Knowledge Solutions Ltd.",
    "LTM.NS": "LTIMindtree Ltd.",
    "NETWEB.NS": "Netweb Technologies India Ltd.",
    "PINELABS.NS": "Pine Labs Ltd.",
    "REDINGTON.NS": "Redington Ltd.",
    "SAGILITY.NS": "Sagility Ltd.",
    "SYRMA.NS": "Syrma SGS Tech",
    "RAMCOSYS.NS": "Ramco Systems",
    "AFFLE.NS": "Affle India",
    "STLTECH.NS": "Sterlite Technologies",
    "QUICKHEAL.NS": "Quick Heal Tech",
    "NUCLEUS.NS": "Nucleus Software",
    
    # ── PHARMA & HEALTHCARE ────────────────────────────────────────────────────
    "SUNPHARMA.NS": "Sun Pharma",
    "DRREDDY.NS": "Dr. Reddy's",
    "CIPLA.NS": "Cipla",
    "DIVISLAB.NS": "Divi's Labs",
    "LUPIN.NS": "Lupin",
    "TORNTPHARM.NS": "Torrent Pharma",
    "BIOCON.NS": "Biocon",
    "AUROPHARMA.NS": "Aurobindo Pharma",
    "GLAND.NS": "Gland Pharma",
    "SYNGENE.NS": "Syngene Int.",
    "ZYDUSLIFE.NS": "Zydus Lifesciences",
    "ALKEM.NS": "Alkem Labs",
    "IPCALAB.NS": "IPCA Labs",
    "LAURUSLABS.NS": "Laurus Labs",
    "GRANULES.NS": "Granules India",
    "NATCOPHARM.NS": "Natco Pharma",
    "MANKIND.NS": "Mankind Pharma",
    "AJANTPHARM.NS": "Ajanta Pharma",
    "GLENMARK.NS": "Glenmark Pharma",
    "SANOFI.NS": "Sanofi India",
    "JBCHEPHARM.NS": "JB Chemicals",
    "CAPLIPOINT.NS": "Caplin Point Lab",
    "ANTHEM.NS": "Anthem Biosciences Ltd.",
    "BLUEJET.NS": "Blue Jet Healthcare Ltd.",
    "COHANCE.NS": "Cohance Lifesciences Ltd.",
    "EMCURE.NS": "Emcure Pharmaceuticals Ltd.",
    "ERIS.NS": "Eris Lifesciences Ltd.",
    "JUBLPHARMA.NS": "Jubilant Pharmova Ltd.",
    "NEULANDLAB.NS": "Neuland Laboratories Ltd.",
    "ONESOURCE.NS": "Onesource Specialty Pharma Ltd.",
    "PPLPHARMA.NS": "Piramal Pharma Ltd.",
    "POLYMED.NS": "Poly Medicure Ltd.",
    "SAILIFE.NS": "Sai Life Sciences Ltd.",
    "WOCKPHARMA.NS": "Wockhardt Ltd.",
    "PFIZER.NS": "Pfizer India",
    "GLAXO.NS": "GSK Pharma India",
    "SOLARA.NS": "Solara Active Pharma",
    "ASTRAZEN.NS": "AstraZeneca India",
    "SUVEN.NS": "Suven Pharma",
    "UNICHEMLAB.NS": "Unichem Laboratories",
    "HESTERBIO.NS": "Hester Biosciences",
    "IOLCP.NS": "IOL Chemicals",
    "STAR.NS": "Strides Pharma",
    
    # ── HOSPITALS & DIAGNOSTICS ────────────────────────────────────────────────
    "APOLLOHOSP.NS": "Apollo Hospitals",
    "MAXHEALTH.NS": "Max Healthcare",
    "FORTIS.NS": "Fortis Healthcare",
    "ABBOTINDIA.NS": "Abbott India",
    "LALPATHLAB.NS": "Dr Lal PathLabs",
    "METROPOLIS.NS": "Metropolis Healthcare",
    "CONCORDBIO.NS": "Concord Biotech",
    "THYROCARE.NS": "Thyrocare Tech",
    "KIMS.NS": "KIMS Hospitals",
    "RAINBOW.NS": "Rainbow Children's",
    "NH.NS": "Narayana Hrudayalaya",
    "VIJAYA.NS": "Vijaya Diagnostic Centre Ltd.",
    "KRSNAA.NS": "Krsnaa Diagnostics",
    "MEDANTA.NS": "Global Health (Medanta)",
    "YATHARTH.NS": "Yatharth Hospital",
    "ASTERDM.NS": "Aster DM Healthcare",
    
    # ── AUTOMOBILE ─────────────────────────────────────────────────────────────
    "MARUTI.NS": "Maruti Suzuki",
    "M&M.NS": "Mahindra & Mah.",
    "BAJAJ-AUTO.NS": "Bajaj Auto",
    "HEROMOTOCO.NS": "Hero MotoCorp",
    "EICHERMOT.NS": "Eicher Motors",
    "TVSMOTOR.NS": "TVS Motor",
    "ASHOKLEY.NS": "Ashok Leyland",
    "ATHERENERG.NS": "Ather Energy Ltd.",
    "FORCEMOT.NS": "Force Motors Ltd.",
    "HYUNDAI.NS": "Hyundai Motor India Ltd.",
    "OLAELEC.NS": "Ola Electric Mobility Ltd.",
    "TMCV.NS": "Tata Motors Ltd.",
    "TMPV.NS": "Tata Motors Passenger Vehicles Ltd.",
    "ESCORTS.NS": "Escorts Kubota",
    
    # ── AUTO ANCILLARIES ───────────────────────────────────────────────────────
    "MOTHERSON.NS": "Samvardhana Mother.",
    "BOSCHLTD.NS": "Bosch",
    "EXIDEIND.NS": "Exide Industries",
    "ARE&M.NS": "Amara Raja Energy",
    "BALKRISIND.NS": "Balkrishna Ind.",
    "APOLLOTYRE.NS": "Apollo Tyres",
    "MRF.NS": "MRF",
    "TIINDIA.NS": "Tube Investments",
    "ENDURANCE.NS": "Endurance Tech",
    "CRAFTSMAN.NS": "Craftsman Auto",
    "SUPRAJIT.NS": "Suprajit Engg.",
    "SONACOMS.NS": "Sona BLW Precision",
    "UNOMINDA.NS": "Uno Minda",
    "GABRIEL.NS": "Gabriel India",
    "SUNDRMFAST.NS": "Sundram Fasteners",
    "BEML.NS": "BEML",
    "JKTYRE.NS": "JK Tyre",
    "CEATLTD.NS": "CEAT",
    "ASAHIINDIA.NS": "Asahi India Glass Ltd.",
    "BELRISE.NS": "Belrise Industries Ltd.",
    "JBMA.NS": "JBM Auto Ltd.",
    "MSUMI.NS": "Motherson Sumi Wiring India Ltd.",
    "RKFORGE.NS": "Ramkrishna Forgings Ltd.",
    "TENNIND.NS": "Tenneco Clean Air India Ltd.",
    "ZFCVINDIA.NS": "ZF Commercial Vehicle Control Systems India Ltd.",
    "JTEKTINDIA.NS": "JTEKT India",
    "VARROC.NS": "Varroc Engineering",
    "MINDACORP.NS": "Minda Corp",
    
    # ── CONSUMER GOODS & FMCG ──────────────────────────────────────────────────
    "HINDUNILVR.NS": "Hindustan Unilever",
    "ITC.NS": "ITC",
    "NESTLEIND.NS": "Nestle India",
    "BRITANNIA.NS": "Britannia",
    "MARICO.NS": "Marico",
    "GODREJCP.NS": "Godrej Consumer",
    "COLPAL.NS": "Colgate-Palmolive",
    "DABUR.NS": "Dabur India",
    "EMAMILTD.NS": "Emami",
    "TATACONSUM.NS": "Tata Consumer",
    "VBL.NS": "Varun Beverages",
    "RADICO.NS": "Radico Khaitan",
    "JUBLFOOD.NS": "Jubilant Foodworks",
    "DEVYANI.NS": "Devyani Intl.",
    "PATANJALI.NS": "Patanjali Foods",
    "BIKAJI.NS": "Bikaji Foods",
    "BATAINDIA.NS": "Bata India",
    "VSTIND.NS": "VST Industries",
    "JYOTHYLAB.NS": "Jyothy Labs",
    "HATSUN.NS": "Hatsun Agro",
    "HERITGFOOD.NS": "Heritage Foods",
    "BAJAJCON.NS": "Bajaj Consumer Care",
    "METROBRAND.NS": "Metro Brands",
    "SAPPHIRE.NS": "Sapphire Foods",
    "AWL.NS": "AWL Agri Business Ltd.",
    "ABDL.NS": "Allied Blenders and Distillers Ltd.",
    "BBTC.NS": "Bombay Burmah Trading Corporation Ltd.",
    "CCL.NS": "CCL Products (I) Ltd.",
    "GODFRYPHLP.NS": "Godfrey Phillips India Ltd.",
    "HONASA.NS": "Honasa Consumer Ltd.",
    "IGIL.NS": "International Gemmological Institute (India) Ltd.",
    "LGEINDIA.NS": "LG Electronics India Ltd.",
    "UBL.NS": "United Breweries Ltd.",
    "UNITDSPR.NS": "United Spirits Ltd.",
    "ZYDUSWELL.NS": "Zydus Wellness Ltd.",
    "GILLETTE.NS": "Gillette India",
    "AVANTIFEED.NS": "Avanti Feeds",
    "VENKEYS.NS": "Venky's India",
    
    # ── METALS & MINING ────────────────────────────────────────────────────────
    "JSWSTEEL.NS": "JSW Steel",
    "TATASTEEL.NS": "Tata Steel",
    "HINDALCO.NS": "Hindalco",
    "VEDL.NS": "Vedanta",
    "SAIL.NS": "SAIL",
    "NMDC.NS": "NMDC",
    "COALINDIA.NS": "Coal India",
    "NATIONALUM.NS": "National Aluminium",
    "HINDCOPPER.NS": "Hindustan Copper",
    "APLAPOLLO.NS": "APL Apollo Tubes",
    "WELCORP.NS": "Welspun Corp",
    "JSL.NS": "Jindal Stainless",
    "JINDALSTEL.NS": "Jindal Steel & Pwr",
    "RATNAMANI.NS": "Ratnamani Metals",
    "SHYAMMETL.NS": "Shyam Metalics",
    "MOIL.NS": "MOIL",
    "MIDHANI.NS": "Mishra Dhatu Nigam",
    "GALLANTT.NS": "Gallantt Ispat Ltd.",
    "GRAPHITE.NS": "Graphite India Ltd.",
    "GRAVITA.NS": "Gravita India Ltd.",
    "GMDCLTD.NS": "Gujarat Mineral Development Corporation Ltd.",
    "HEG.NS": "H.E.G. Ltd.",
    "JAINREC.NS": "Jain Resource Recycling Ltd.",
    "JINDALSAW.NS": "Jindal Saw Ltd.",
    "LLOYDSME.NS": "Lloyds Metals And Energy Ltd.",
    "NSLNISP.NS": "NMDC Steel Ltd.",
    "NAVA.NS": "Nava Ltd.",
    "RHIM.NS": "RHI MAGNESITA INDIA LTD.",
    "SARDAEN.NS": "Sarda Energy and Minerals Ltd.",
    "USHAMART.NS": "Usha Martin Ltd.",
    "GPPL.NS": "Gujarat Pipavav Port",
    "MMTC.NS": "MMTC",
    "HINDZINC.NS": "Hindustan Zinc",
    "GPIL.NS": "Godawari Power",
    "SUNFLAG.NS": "Sunflag Iron",
    
    # ── ENERGY & OIL-GAS ───────────────────────────────────────────────────────
    "RELIANCE.NS": "Reliance Ind.",
    "ONGC.NS": "ONGC",
    "BPCL.NS": "BPCL",
    "IOC.NS": "Indian Oil",
    "GAIL.NS": "GAIL",
    "PETRONET.NS": "Petronet LNG",
    "IGL.NS": "Indraprastha Gas",
    "MGL.NS": "Mahanagar Gas",
    "CHENNPETRO.NS": "Chennai Petroleum Corporation Ltd.",
    "GSPL.NS": "Gujarat State Petronet Ltd.",
    "CASTROLIND.NS": "Castrol India Ltd.",
    "HINDPETRO.NS": "HPCL",
    "MRPL.NS": "MRPL",
    "OIL.NS": "Oil India",
    
    # ── POWER & UTILITIES ──────────────────────────────────────────────────────
    "NTPC.NS": "NTPC",
    "POWERGRID.NS": "Power Grid",
    "TATAPOWER.NS": "Tata Power",
    "ADANIGREEN.NS": "Adani Green",
    "ADANIPOWER.NS": "Adani Power",
    "CESC.NS": "CESC",
    "TORNTPOWER.NS": "Torrent Power",
    "ATGL.NS": "Adani Total Gas",
    "NHPC.NS": "NHPC",
    "SJVN.NS": "SJVN",
    "INOXWIND.NS": "Inox Wind",
    "SUZLON.NS": "Suzlon Energy",
    "JSWENERGY.NS": "JSW Energy",
    "GIPCL.NS": "GIPCL",
    "ACMESOLAR.NS": "ACME Solar Holdings Ltd.",
    "ADANIENSOL.NS": "Adani Energy Solutions Ltd.",
    "EMMVEE.NS": "Emmvee Photovoltaic Power Ltd.",
    "JPPOWER.NS": "Jaiprakash Power Ventures Ltd.",
    "NLCINDIA.NS": "NLC India Ltd.",
    "NTPCGREEN.NS": "NTPC Green Energy Ltd.",
    "PREMIERENE.NS": "Premier Energies Ltd.",
    "RPOWER.NS": "Reliance Power Ltd.",
    "WAAREEENER.NS": "Waaree Energies Ltd.",
    
    # ── CEMENT & BUILDING MATERIALS ───────────────────────────────────────────
    "ULTRACEMCO.NS": "UltraTech Cement",
    "SHREECEM.NS": "Shree Cement",
    "AMBUJACEM.NS": "Ambuja Cement",
    "GRASIM.NS": "Grasim Ind.",
    "DALBHARAT.NS": "Dalmia Bharat",
    "JKCEMENT.NS": "JK Cement",
    "RAMCOCEM.NS": "Ramco Cements",
    "HEIDELBERG.NS": "Heidelberg Cement",
    "NUVOCO.NS": "Nuvoco Vistas",
    "JKPAPER.NS": "JK Paper",
    "ACC.NS": "ACC Ltd.",
    "INDIACEM.NS": "India Cements Ltd.",
    "JSWCEMENT.NS": "JSW Cement Ltd.",
    "KAJARIACER.NS": "Kajaria Ceramics Ltd.",
    "CENTURYPLY.NS": "Century Plyboards",
    "GREENPLY.NS": "Greenply Industries",
    "GREENLAM.NS": "Greenlam Industries",
    "CERA.NS": "Cera Sanitaryware",
    "SOMANYCERA.NS": "Somany Ceramics",
    "ORIENTBELL.NS": "Orient Bell",
    
    # ── CAPITAL GOODS & ENGINEERING ────────────────────────────────────────────
    "LT.NS": "L&T",
    "SIEMENS.NS": "Siemens",
    "ABB.NS": "ABB India",
    "BHEL.NS": "BHEL",
    "CGPOWER.NS": "CG Power",
    "THERMAX.NS": "Thermax",
    "BHARATFORG.NS": "Bharat Forge",
    "GRINDWELL.NS": "Grindwell Norton",
    "HAVELLS.NS": "Havells India",
    "POLYCAB.NS": "Polycab India",
    "VOLTAS.NS": "Voltas",
    "CROMPTON.NS": "Crompton Greaves",
    "AMBER.NS": "Amber Enterprises",
    "CUMMINSIND.NS": "Cummins India",
    "SCHAEFFLER.NS": "Schaeffler India",
    "TIMKEN.NS": "Timken India",
    "SKFINDIA.NS": "SKF India",
    "ELGIEQUIP.NS": "Elgi Equipments",
    "PRAJIND.NS": "Praj Industries",
    "AIAENG.NS": "AIA Engineering",
    "APARINDS.NS": "Apar Industries",
    "CARBORUNIV.NS": "Carborundum Universal",
    "VGUARD.NS": "V-Guard Industries",
    "WHIRLPOOL.NS": "Whirlpool India",
    "BLUESTARCO.NS": "Blue Star",
    "KEC.NS": "KEC International",
    "KALPATARU.NS": "Kalpataru Power",
    "TITAGARH.NS": "Titagarh Rail",
    "ACE.NS": "Action Construction Equipment Ltd.",
    "DIXON.NS": "Dixon Technologies (India) Ltd.",
    "GVT&D.NS": "GE Vernova T&D India Ltd.",
    "HBLENGINE.NS": "HBL Engineering Ltd.",
    "POWERINDIA.NS": "Hitachi Energy India Ltd.",
    "JWL.NS": "Jupiter Wagons Ltd.",
    "JYOTICNC.NS": "Jyoti CNC Automation Ltd.",
    "KEI.NS": "KEI Industries Ltd.",
    "PGEL.NS": "PG Electroplast Ltd.",
    "PTCIL.NS": "PTC Industries Ltd.",
    "RRKABEL.NS": "R R Kabel Ltd.",
    "SCHNEIDER.NS": "Schneider Electric Infrastructure Ltd.",
    "ENRIN.NS": "Siemens Energy India Ltd.",
    "TECHNOE.NS": "Techno Electric & Engineering Company Ltd.",
    "TEGA.NS": "Tega Industries Ltd.",
    "TARIL.NS": "Transformers And Rectifiers (India) Ltd.",
    "TRITURBINE.NS": "Triveni Turbine Ltd.",
    "HONAUT.NS": "Honeywell Automation",
    "3MINDIA.NS": "3M India",
    "INOXINDIA.NS": "INOX India",
    "POWERMECH.NS": "Power Mech Projects",
    "ISGEC.NS": "ISGEC Heavy Engg.",
    "HFCL.NS": "HFCL",
    "TDPOWERSYS.NS": "TD Power Systems",
    "ELECON.NS": "Elecon Engineering",
    "BAJAJELEC.NS": "Bajaj Electricals",
    "HAPPYFORGE.NS": "Happy Forgings",
    "SUPREMEIND.NS": "Supreme Industries",
    "FINCABLES.NS": "Finolex Cables",
    "FINPIPE.NS": "Finolex Industries",
    "APOLLOPIPE.NS": "Apollo Pipes",
    "PRINCEPIPE.NS": "Prince Pipes",
    
    # ── DEFENCE & PSU MANUFACTURING ────────────────────────────────────────────
    "BEL.NS": "Bharat Electronics",
    "HAL.NS": "Hindustan Aeronautics",
    "MAZDOCK.NS": "Mazagon Dock",
    "COCHINSHIP.NS": "Cochin Shipyard",
    "GRSE.NS": "Garden Reach Ship.",
    "DATAPATTNS.NS": "Data Patterns",
    "ZENTEC.NS": "Zen Technologies Ltd.",
    "BDL.NS": "Bharat Dynamics",
    "SOLARINDS.NS": "Solar Industries India",
    
    # ── INFRASTRUCTURE & CONSTRUCTION ──────────────────────────────────────────
    "CONCOR.NS": "Container Corp.",
    "IRFC.NS": "IRFC",
    "RVNL.NS": "Rail Vikas Nigam",
    "IRCON.NS": "IRCON Intl.",
    "NBCC.NS": "NBCC India",
    "RAILTEL.NS": "RailTel Corp.",
    "HUDCO.NS": "HUDCO",
    "PNCINFRA.NS": "PNC Infratech",
    "IRB.NS": "IRB Infrastructure",
    "ASHOKA.NS": "Ashoka Buildcon",
    "JKIL.NS": "J. Kumar Infraprojects",
    "AFCONS.NS": "Afcons Infrastructure Ltd.",
    "CEMPRO.NS": "Cemindia Projects Ltd.",
    "ENGINERSIN.NS": "Engineers India Ltd.",
    "GMRAIRPORT.NS": "GMR Airports Ltd.",
    "KPIL.NS": "Kalpataru Projects International Ltd.",
    "NCC.NS": "NCC Ltd.",
    "RITES.NS": "RITES",
    "PSPPROJECT.NS": "PSP Projects",
    "HGINFRA.NS": "HG Infra Engineering",
    
    # ── CHEMICALS & SPECIALTY ──────────────────────────────────────────────────
    "PIDILITIND.NS": "Pidilite Ind.",
    "ASTRAL.NS": "Astral",
    "PIIND.NS": "PI Industries",
    "SRF.NS": "SRF",
    "DEEPAKNTR.NS": "Deepak Nitrite",
    "GNFC.NS": "GNFC",
    "TATACHEM.NS": "Tata Chemicals",
    "AARTIIND.NS": "Aarti Industries",
    "NAVINFLUOR.NS": "Navin Fluorine",
    "ATUL.NS": "Atul",
    "FINEORG.NS": "Fine Organics",
    "ALKYLAMINE.NS": "Alkyl Amines",
    "PCBL.NS": "PCBL",
    "GALAXYSURF.NS": "Galaxy Surfactants",
    "LXCHEM.NS": "Laxmi Organic",
    "CLEAN.NS": "Clean Science",
    "LINDEINDIA.NS": "Linde India",
    "VINATIORGA.NS": "Vinati Organics",
    "NOCIL.NS": "NOCIL",
    "ROSSARI.NS": "Rossari Biotech",
    "SUDARSCHEM.NS": "Sudarshan Chemical",
    "DCMSHRIRAM.NS": "DCM Shriram",
    "ACUTAAS.NS": "Acutaas Chemicals Ltd.",
    "ANURAS.NS": "Anupam Rasayan India Ltd.",
    "FLUOROCHEM.NS": "Gujarat Fluorochemicals Ltd.",
    "GFLLIMITED.NS": "GFL Ltd.",
    "HSCL.NS": "Himadri Speciality Chemical Ltd.",
    "JUBLINGREA.NS": "Jubilant Ingrevia Ltd.",
    "SPLPETRO.NS": "Supreme Petrochem Ltd.",
    "BALAMINES.NS": "Balaji Amines",
    "BASF.NS": "BASF India",
    "TATVA.NS": "Tatva Chintan Pharma",
    "TARSONS.NS": "Tarsons Products",
    
    # ── AGRI, FERTILISERS & CROP SCIENCE ──────────────────────────────────────
    "COROMANDEL.NS": "Coromandel Intl.",
    "RALLIS.NS": "Rallis India",
    "SUMICHEM.NS": "Sumitomo Chemical",
    "GHCL.NS": "GHCL",
    "DEEPAKFERT.NS": "Deepak Fertilizers",
    "EIDPARRY.NS": "EID Parry",
    "BAYERCROP.NS": "Bayer CropScience",
    "GODREJAGRO.NS": "Godrej Agrovet",
    "UPL.NS": "UPL",
    "CHAMBLFERT.NS": "Chambal Fertilizers & Chemicals Ltd.",
    "FACT.NS": "Fertilisers and Chemicals Travancore Ltd.",
    "PARADEEP.NS": "Paradeep Phosphates Ltd.",
    "KRBL.NS": "KRBL",
    "LTFOODS.NS": "LT Foods",
    "BALRAMCHIN.NS": "Balrampur Chini",
    "TRIVENI.NS": "Triveni Engineering",
    "KSCL.NS": "Kaveri Seed",
    "DHANUKA.NS": "Dhanuka Agritech",
    
    # ── PAINTS ────────────────────────────────────────────────────────────────
    "ASIANPAINT.NS": "Asian Paints",
    "BERGEPAINT.NS": "Berger Paints",
    "KANSAINER.NS": "Kansai Nerolac",
    "AKZOINDIA.NS": "Akzo Nobel",
    
    # ── REAL ESTATE & CONSTRUCTION ─────────────────────────────────────────────
    "DLF.NS": "DLF",
    "GODREJPROP.NS": "Godrej Properties",
    "OBEROIRLTY.NS": "Oberoi Realty",
    "PRESTIGE.NS": "Prestige Estates",
    "BRIGADE.NS": "Brigade Enterprises",
    "PHOENIXLTD.NS": "Phoenix Mills",
    "SOBHA.NS": "Sobha",
    "MAHLIFE.NS": "Mahindra Lifespace",
    "ANANTRAJ.NS": "Anant Raj",
    "KOLTEPATIL.NS": "Kolte-Patil Dev.",
    "SIGNATURE.NS": "Signatureglobal",
    "JSWINFRA.NS": "JSW Infrastructure",
    "ABREL.NS": "Aditya Birla Real Estate Ltd.",
    "LODHA.NS": "Lodha Developers Ltd.",
    "SUNTECK.NS": "Sunteck Realty",
    "PURVA.NS": "Puravankara",
    
    # ── TELECOM ────────────────────────────────────────────────────────────────
    "BHARTIARTL.NS": "Bharti Airtel",
    "INDUSTOWER.NS": "Indus Towers",
    "TATACOMM.NS": "Tata Communications",
    "BHARTIHEXA.NS": "Bharti Hexacom Ltd.",
    "ITI.NS": "ITI Ltd.",
    "TTML.NS": "Tata Teleservices (Maharashtra) Ltd.",
    "TEJASNET.NS": "Tejas Networks Ltd.",
    "IDEA.NS": "Vodafone Idea Ltd.",
    
    # ── LOGISTICS & TRANSPORT ──────────────────────────────────────────────────
    "IRCTC.NS": "IRCTC",
    "DELHIVERY.NS": "Delhivery",
    "ALLCARGO.NS": "Allcargo Logistics",
    "BLUEDART.NS": "Blue Dart",
    "VRLLOG.NS": "VRL Logistics",
    "TCI.NS": "Transport Corp.",
    "MAHLOG.NS": "Mahindra Logistics",
    "TCIEXP.NS": "TCI Express",
    "AEGISLOG.NS": "Aegis Logistics Ltd.",
    "AEGISVOPAK.NS": "Aegis Vopak Terminals Ltd.",
    "GESHIP.NS": "Great Eastern Shipping Co. Ltd.",
    "INDIGO.NS": "InterGlobe Aviation Ltd.",
    "SCI.NS": "Shipping Corporation of India Ltd.",
    "GDLLEAS.NS": "Gateway Distriparks",
    "SNOWMAN.NS": "Snowman Logistics",
    
    # ── RETAIL & CONSUMER DISCRETIONARY ───────────────────────────────────────
    "TITAN.NS": "Titan",
    "TRENT.NS": "Trent",
    "DMART.NS": "Avenue Supermarts",
    "NYKAA.NS": "Nykaa",
    "ETERNAL.NS": "Eternal Limited",
    "PAYTM.NS": "Paytm",
    "POLICYBZR.NS": "PB Fintech",
    "KALYANKJIL.NS": "Kalyan Jewellers",
    "SENCO.NS": "Senco Gold",
    "DOMS.NS": "DOMS Industries",
    "EASEMYTRIP.NS": "EaseMyTrip",
    "WONDERLA.NS": "Wonderla Holidays",
    "UFBL.NS": "United Foodbrands",
    "MANYAVAR.NS": "Vedant Fashions",
    "OLECTRA.NS": "Olectra Greentech",
    "ABFRL.NS": "Aditya Birla Fashion and Retail Ltd.",
    "ABLBL.NS": "Aditya Birla Lifestyle Brands Ltd.",
    "FIRSTCRY.NS": "Brainbees Solutions Ltd.",
    "LENSKART.NS": "Lenskart Solutions Ltd.",
    "MEESHO.NS": "Meesho Ltd.",
    "SWIGGY.NS": "Swiggy Ltd.",
    "VMM.NS": "Vishal Mega Mart Ltd.",
    "VAIBHAVGBL.NS": "Vaibhav Global",
    
    # ── TEXTILES & APPAREL ────────────────────────────────────────────────────
    "PAGEIND.NS": "Page Industries",
    "RAYMOND.NS": "Raymond",
    "ARVIND.NS": "Arvind",
    "TRIDENT.NS": "Trident",
    "KPRMILL.NS": "KPR Mill",
    "VTL.NS": "Vardhman Textiles Ltd.",
    "WELSPUNLIV.NS": "Welspun Living Ltd.",
    "GOKEX.NS": "Gokaldas Exports",
    "RUPA.NS": "Rupa & Company",
    "DOLLAR.NS": "Dollar Industries",
    
    # ── MEDIA & ENTERTAINMENT ──────────────────────────────────────────────────
    "PVRINOX.NS": "PVR Inox",
    "ZEEL.NS": "Zee Entertainment",
    "SUNTV.NS": "Sun TV Network",
    "NAZARA.NS": "Nazara Technologies",
    "SAREGAMA.NS": "Saregama India",
    "NETWORK18.NS": "Network18 Media",
    
    # ── HOSPITALITY & TRAVEL ──────────────────────────────────────────────────
    "INDHOTEL.NS": "Indian Hotels",
    "LEMONTREE.NS": "Lemon Tree Hotels",
    "CHALET.NS": "Chalet Hotels",
    "JUNIPER.NS": "Juniper Hotels",
    "EIHOTEL.NS": "EIH Ltd.",
    "ITCHOTELS.NS": "ITC Hotels Ltd.",
    "THELEELA.NS": "Leela Palaces Hotels & Resorts Ltd.",
    "TBOTEK.NS": "TBO Tek Ltd.",
    "TRAVELFOOD.NS": "Travel Food Services Ltd.",
    
    # ── CONGLOMERATES ─────────────────────────────────────────────────────────
    "ADANIENT.NS": "Adani Enterprises",
    "ADANIPORTS.NS": "Adani Ports",
    "GODREJIND.NS": "Godrej Industries",
    "BAJAJHLDNG.NS": "Bajaj Holdings",
    "SWANCORP.NS": "Swan Corp Ltd.",
    "JSWHL.NS": "JSW Holdings",
    "TATAINVEST.NS": "Tata Investment Corp",
    
    # ── EDUCATION & MISC ──────────────────────────────────────────────────────
    "JUSTDIAL.NS": "Just Dial",
    "INDIAMART.NS": "IndiaMART",
    "KAYNES.NS": "Kaynes Technology",
    "RAJESHEXPO.NS": "Rajesh Exports",
    "POLYPLEX.NS": "Polyplex Corp.",
    "MTARTECH.NS": "MTAR Technologies",
    "BLS.NS": "BLS International Services Ltd.",
    "PWL.NS": "Physicswallah Ltd.",
    "URBANCO.NS": "Urban Company Ltd.",
}

# Python dicts reject duplicate keys at parse time; this line is a safety net.
STOCKS = {k: v for k, v in STOCKS.items()}


# ── Sector mapping ─────────────────────────────────────────────────────────────
# Keys are base tickers WITHOUT the .NS suffix.
# Used by the backtest sector-concentration cap (MAX_TRADES_PER_SECTOR).
# Unmapped tickers fall back to "OTHER".

SECTORS: dict[str, str] = {
    # BANKING
    "HDFCBANK": "BANKING",
    "ICICIBANK": "BANKING",
    "KOTAKBANK": "BANKING",
    "SBIN": "BANKING",
    "AXISBANK": "BANKING",
    "INDUSINDBK": "BANKING",
    "BANDHANBNK": "BANKING",
    "FEDERALBNK": "BANKING",
    "IDFCFIRSTB": "BANKING",
    "RBLBANK": "BANKING",
    "BANKBARODA": "BANKING",
    "PNB": "BANKING",
    "CANARABANK": "BANKING",
    "UNIONBANK": "BANKING",
    "INDIANB": "BANKING",
    "AUBANK": "BANKING",
    "EQUITASBNK": "BANKING",
    "UJJIVANSFB": "BANKING",
    "IDBI": "BANKING",
    "KTKBANK": "BANKING",
    "DCBBANK": "BANKING",
    "CSBBANK": "BANKING",
    "BANKINDIA": "BANKING",
    "MAHABANK": "BANKING",
    "CENTRALBK": "BANKING",
    "IOB": "BANKING",
    "UCOBANK": "BANKING",
    "YESBANK": "BANKING",
    "J&KBANK": "BANKING",
    "CUB": "BANKING",
    "KARURVYSYA": "BANKING",
    "SOUTHBANK": "BANKING",
    "CITYUNIONBK": "BANKING",

    # NBFC, HOUSING FINANCE & FINANCIAL SERVICES
    "BAJFINANCE": "NBFC",
    "BAJAJFINSV": "NBFC",
    "CHOLAFIN": "NBFC",
    "MUTHOOTFIN": "NBFC",
    "MANAPPURAM": "NBFC",
    "ABCAPITAL": "NBFC",
    "SBICARD": "NBFC",
    "SHRIRAMFIN": "NBFC",
    "M&MFIN": "NBFC",
    "IIFLFINANCE": "NBFC",
    "POONAWALLA": "NBFC",
    "MASFIN": "NBFC",
    "CREDITACC": "NBFC",
    "5PAISA": "NBFC",
    "360ONE": "NBFC",
    "AADHARHFC": "NBFC",
    "AIIL": "NBFC",
    "BAJAJHFL": "NBFC",
    "CANFINHOME": "NBFC",
    "CGCL": "NBFC",
    "FIVESTAR": "NBFC",
    "HDBFS": "NBFC",
    "IFCI": "NBFC",
    "LTF": "NBFC",
    "LICHSGFIN": "NBFC",
    "PIRAMALFIN": "NBFC",
    "PFC": "NBFC",
    "SBFC": "NBFC",
    "SAMMAANCAP": "NBFC",
    "TATACAP": "NBFC",
    "SUNDARMFIN": "NBFC",
    "JMFINANCIL": "NBFC",
    "MFSL": "NBFC",
    "CHOLAHLDNG": "NBFC",
    "IIFLCAPS": "NBFC",
    "RECLTD": "NBFC",
    "IREDA": "NBFC",
    "AAVAS": "HOUSING_FINANCE",
    "HOMEFIRST": "HOUSING_FINANCE",
    "APTUS": "HOUSING_FINANCE",
    "PNBHOUSING": "HOUSING_FINANCE",
    "REPCOHOME": "HOUSING_FINANCE",
    "SPANDANA": "NBFC",
    "FUSION": "NBFC",

    # INSURANCE
    "LICI": "INSURANCE",
    "SBILIFE": "INSURANCE",
    "HDFCLIFE": "INSURANCE",
    "ICICIGI": "INSURANCE",
    "ICICIPRULI": "INSURANCE",
    "STARHEALTH": "INSURANCE",
    "JIOFIN": "INSURANCE",
    "CANHLIFE": "INSURANCE",
    "GICRE": "INSURANCE",
    "GODIGIT": "INSURANCE",
    "NIVABUPA": "INSURANCE",
    "NIACL": "INSURANCE",

    # AMC & CAPITAL MARKETS
    "HDFCAMC": "AMC",
    "UTIAMC": "AMC",
    "NIPPONLIFE": "AMC",
    "CAMS": "AMC",
    "KFINTECH": "AMC",
    "ABSLAMC": "AMC",
    "ICICIAMC": "AMC",
    "BSE": "CAP_MARKETS",
    "MCX": "CAP_MARKETS",
    "CDSL": "CAP_MARKETS",
    "CRISIL": "CAP_MARKETS",
    "ANGELONE": "CAP_MARKETS",
    "MOFSL": "CAP_MARKETS",
    "ICRA": "CAP_MARKETS",
    "NUVAMA": "CAP_MARKETS",
    "ANANDRATHI": "CAP_MARKETS",
    "GROWW": "CAP_MARKETS",
    "CHOICEIN": "CAP_MARKETS",
    "IEX": "CAP_MARKETS",

    # INFORMATION TECHNOLOGY
    "TCS": "IT",
    "INFY": "IT",
    "WIPRO": "IT",
    "HCLTECH": "IT",
    "TECHM": "IT",
    "LTM": "IT",
    "LTTS": "IT",
    "PERSISTENT": "IT",
    "COFORGE": "IT",
    "MPHASIS": "IT",
    "KPITTECH": "IT",
    "TATAELXSI": "IT",
    "OFSS": "IT",
    "CYIENT": "IT",
    "NAUKRI": "IT",
    "MASTEK": "IT",
    "HAPPSTMNDS": "IT",
    "TANLA": "IT",
    "NEWGEN": "IT",
    "LATENTVIEW": "IT",
    "BSOFT": "IT",
    "SONATSOFTW": "IT",
    "INTELLECT": "IT",
    "TATATECH": "IT",
    "ZENSAR": "IT",
    "RATEGAIN": "IT",
    "HEXAWARE": "IT",
    "ECLERX": "IT",
    "DATAMATICS": "IT",
    "ROUTE": "IT",
    "CPPLUS": "IT",
    "MAPMYINDIA": "IT",
    "CARTRADE": "IT",
    "FSL": "IT",
    "INDGN": "IT",
    "IKS": "IT",
    "NETWEB": "IT",
    "PINELABS": "IT",
    "REDINGTON": "IT",
    "SAGILITY": "IT",
    "SYRMA": "IT",
    "RAMCOSYS": "IT",
    "AFFLE": "IT",
    "STLTECH": "IT",
    "QUICKHEAL": "IT",
    "NUCLEUS": "IT",

    # PHARMA & HEALTHCARE
    "SUNPHARMA": "PHARMA",
    "DRREDDY": "PHARMA",
    "CIPLA": "PHARMA",
    "DIVISLAB": "PHARMA",
    "LUPIN": "PHARMA",
    "TORNTPHARM": "PHARMA",
    "BIOCON": "PHARMA",
    "AUROPHARMA": "PHARMA",
    "GLAND": "PHARMA",
    "SYNGENE": "PHARMA",
    "ZYDUSLIFE": "PHARMA",
    "ALKEM": "PHARMA",
    "IPCALAB": "PHARMA",
    "LAURUSLABS": "PHARMA",
    "GRANULES": "PHARMA",
    "NATCOPHARM": "PHARMA",
    "MANKIND": "PHARMA",
    "AJANTPHARM": "PHARMA",
    "GLENMARK": "PHARMA",
    "SANOFI": "PHARMA",
    "JBCHEPHARM": "PHARMA",
    "CAPLIPOINT": "PHARMA",
    "SEQUENT": "PHARMA",
    "ANTHEM": "PHARMA",
    "BLUEJET": "PHARMA",
    "COHANCE": "PHARMA",
    "EMCURE": "PHARMA",
    "ERIS": "PHARMA",
    "JUBLPHARMA": "PHARMA",
    "NEULANDLAB": "PHARMA",
    "ONESOURCE": "PHARMA",
    "PPLPHARMA": "PHARMA",
    "POLYMED": "HEALTHCARE",
    "SAILIFE": "PHARMA",
    "WOCKPHARMA": "PHARMA",
    "PFIZER": "PHARMA",
    "GLAXO": "PHARMA",
    "SOLARA": "PHARMA",
    "ASTRAZEN": "PHARMA",
    "SUVEN": "PHARMA",
    "UNICHEMLAB": "PHARMA",
    "HESTERBIO": "PHARMA",
    "IOLCP": "PHARMA",
    "STAR": "PHARMA",

    # HOSPITALS & DIAGNOSTICS
    "APOLLOHOSP": "HOSPITALS",
    "MAXHEALTH": "HOSPITALS",
    "FORTIS": "HOSPITALS",
    "ABBOTINDIA": "HOSPITALS",
    "LALPATHLAB": "HOSPITALS",
    "METROPOLIS": "HOSPITALS",
    "CONCORD": "HOSPITALS",
    "THYROCARE": "HOSPITALS",
    "KIMS": "HOSPITALS",
    "RAINBOW": "HOSPITALS",
    "NH": "HOSPITALS",
    "VIJAYA": "HOSPITALS",
    "KRSNAA": "HOSPITALS",
    "MEDANTA": "HOSPITALS",
    "YATHARTH": "HEALTHCARE",
    "ASTERDM": "HEALTHCARE",

    # AUTOMOBILE (OEM)
    "MARUTI": "AUTO",
    "M&M": "AUTO",
    "BAJAJ-AUTO": "AUTO",
    "HEROMOTOCO": "AUTO",
    "EICHERMOT": "AUTO",
    "TVSMOTOR": "AUTO",
    "ASHOKLEY": "AUTO",
    "ATHERENERG": "AUTO",
    "FORCEMOT": "AUTO",
    "HYUNDAI": "AUTO",
    "OLAELEC": "AUTO",
    "TMCV": "AUTO",
    "TMPV": "AUTO",
    "ESCORTS": "AUTO",

    # AUTO ANCILLARIES
    "MOTHERSON": "AUTO_ANC",
    "BOSCHLTD": "AUTO_ANC",
    "EXIDEIND": "AUTO_ANC",
    "AMARARAJA": "AUTO_ANC",
    "BALKRISIND": "AUTO_ANC",
    "APOLLOTYRE": "AUTO_ANC",
    "MRF": "AUTO_ANC",
    "TIINDIA": "AUTO_ANC",
    "ENDURANCE": "AUTO_ANC",
    "CRAFTSMAN": "AUTO_ANC",
    "SUPRAJIT": "AUTO_ANC",
    "SONACOMS": "AUTO_ANC",
    "UNOMINDA": "AUTO_ANC",
    "MAHINDCIE": "AUTO_ANC",
    "GABRIEL": "AUTO_ANC",
    "SUNDRMFAST": "AUTO_ANC",
    "BEML": "AUTO_ANC",
    "JKTYRE": "AUTO_ANC",
    "CEATLTD": "AUTO_ANC",
    "ASAHIINDIA": "AUTO_ANC",
    "BELRISE": "AUTO_ANC",
    "JBMA": "AUTO_ANC",
    "MSUMI": "AUTO_ANC",
    "RKFORGE": "AUTO_ANC",
    "TENNIND": "AUTO_ANC",
    "ZFCVINDIA": "AUTO_ANC",
    "JTEKTINDIA": "AUTO_ANC",
    "VARROC": "AUTO",
    "MINDACORP": "AUTO",

    # FMCG & CONSUMER STAPLES
    "HINDUNILVR": "FMCG",
    "ITC": "FMCG",
    "NESTLEIND": "FMCG",
    "BRITANNIA": "FMCG",
    "MARICO": "FMCG",
    "GODREJCP": "FMCG",
    "COLPAL": "FMCG",
    "DABUR": "FMCG",
    "EMAMILTD": "FMCG",
    "TATACONSUM": "FMCG",
    "VBL": "FMCG",
    "RADICO": "FMCG",
    "PATANJALI": "FMCG",
    "BIKAJI": "FMCG",
    "BATAINDIA": "FMCG",
    "VSTIND": "FMCG",
    "JYOTHYLAB": "FMCG",
    "HATSUN": "FMCG",
    "HERITGFOOD": "FMCG",
    "BAJAJCON": "FMCG",
    "AWL": "FMCG",
    "ABDL": "FMCG",
    "BBTC": "FMCG",
    "CCL": "FMCG",
    "GODFRYPHLP": "FMCG",
    "HONASA": "FMCG",
    "IGIL": "FMCG",
    "LGEINDIA": "FMCG",
    "UBL": "FMCG",
    "UNITDSPR": "FMCG",
    "ZYDUSWELL": "FMCG",
    "GILLETTE": "FMCG",
    "AVANTIFEED": "FMCG",
    "VENKEYS": "FMCG",

    # METALS & MINING
    "JSWSTEEL": "METALS",
    "TATASTEEL": "METALS",
    "HINDALCO": "METALS",
    "VEDL": "METALS",
    "SAIL": "METALS",
    "NMDC": "METALS",
    "COALINDIA": "METALS",
    "NATIONALUM": "METALS",
    "HINDCOPPER": "METALS",
    "APLAPOLLO": "METALS",
    "WELCORP": "METALS",
    "JSL": "METALS",
    "JINDALSTEL": "METALS",
    "RATNAMANI": "METALS",
    "SHYAMMETL": "METALS",
    "MOIL": "METALS",
    "MIDHANI": "METALS",
    "GALLANTT": "METALS",
    "GRAPHITE": "METALS",
    "GRAVITA": "METALS",
    "GMDCLTD": "METALS",
    "HEG": "METALS",
    "JAINREC": "METALS",
    "JINDALSAW": "METALS",
    "LLOYDSME": "METALS",
    "NSLNISP": "METALS",
    "NAVA": "METALS",
    "RHIM": "METALS",
    "SARDAEN": "METALS",
    "USHAMART": "METALS",
    "GPPL": "METALS",
    "MMTC": "METALS",
    "HINDZINC": "METALS",
    "GPIL": "METALS",
    "SUNFLAG": "METALS",

    # OIL & GAS
    "RELIANCE": "OIL_GAS",
    "ONGC": "OIL_GAS",
    "BPCL": "OIL_GAS",
    "IOC": "OIL_GAS",
    "GAIL": "OIL_GAS",
    "PETRONET": "OIL_GAS",
    "IGL": "OIL_GAS",
    "MGL": "OIL_GAS",
    "CHENNPETRO": "OIL_GAS",
    "GSPL": "OIL_GAS",
    "CASTROLIND": "OIL_GAS",
    "HINDPETRO": "OIL_GAS",
    "MRPL": "OIL_GAS",
    "OIL": "OIL_GAS",

    # POWER & UTILITIES
    "NTPC": "POWER",
    "POWERGRID": "POWER",
    "TATAPOWER": "POWER",
    "ADANIGREEN": "POWER",
    "ADANIPOWER": "POWER",
    "CESC": "POWER",
    "TORNTPOWER": "POWER",
    "ATGL": "POWER",
    "NHPC": "POWER",
    "SJVN": "POWER",
    "INOXWIND": "POWER",
    "SUZLON": "POWER",
    "JSWENERGY": "POWER",
    "GIPCL": "POWER",
    "ACMESOLAR": "POWER",
    "ADANIENSOL": "POWER",
    "EMMVEE": "POWER",
    "JPPOWER": "POWER",
    "NLCINDIA": "POWER",
    "NTPCGREEN": "POWER",
    "PREMIERENE": "POWER",
    "RPOWER": "POWER",
    "WAAREEENER": "POWER",

    # CEMENT & BUILDING MATERIALS
    "ULTRACEMCO": "CEMENT",
    "SHREECEM": "CEMENT",
    "AMBUJACEM": "CEMENT",
    "GRASIM": "CEMENT",
    "DALBHARAT": "CEMENT",
    "JKCEMENT": "CEMENT",
    "RAMCOCEM": "CEMENT",
    "HEIDELBERG": "CEMENT",
    "NUVOCO": "CEMENT",
    "JKPAPER": "CEMENT",
    "ACC": "CEMENT",
    "INDIACEM": "CEMENT",
    "JSWCEMENT": "CEMENT",
    "KAJARIACER": "CEMENT",
    "CENTURYPLY": "BUILDING_MATERIALS",
    "GREENPLY": "BUILDING_MATERIALS",
    "GREENLAM": "BUILDING_MATERIALS",
    "CERA": "BUILDING_MATERIALS",
    "SOMANYCERA": "BUILDING_MATERIALS",
    "ORIENTBELL": "BUILDING_MATERIALS",

    # CAPITAL GOODS & ENGINEERING
    "LT": "CAP_GOODS",
    "SIEMENS": "CAP_GOODS",
    "ABB": "CAP_GOODS",
    "BHEL": "CAP_GOODS",
    "CGPOWER": "CAP_GOODS",
    "THERMAX": "CAP_GOODS",
    "BHARATFORG": "CAP_GOODS",
    "GRINDWELL": "CAP_GOODS",
    "HAVELLS": "CAP_GOODS",
    "POLYCAB": "CAP_GOODS",
    "VOLTAS": "CAP_GOODS",
    "CROMPTON": "CAP_GOODS",
    "AMBER": "CAP_GOODS",
    "CUMMINS": "CAP_GOODS",
    "SCHAEFFLER": "CAP_GOODS",
    "TIMKEN": "CAP_GOODS",
    "SKFINDIA": "CAP_GOODS",
    "ELGIEQUIP": "CAP_GOODS",
    "PRAJIND": "CAP_GOODS",
    "AIAENG": "CAP_GOODS",
    "APARINDS": "CAP_GOODS",
    "CARBORUNIV": "CAP_GOODS",
    "VGUARD": "CAP_GOODS",
    "WHIRLPOOL": "CAP_GOODS",
    "BLUESTARCO": "CAP_GOODS",
    "KEC": "CAP_GOODS",
    "KALPATPOWR": "CAP_GOODS",
    "TITAGARH": "CAP_GOODS",
    "ACE": "CAP_GOODS",
    "DIXON": "CAP_GOODS",
    "GVT&D": "CAP_GOODS",
    "HBLENGINE": "CAP_GOODS",
    "POWERINDIA": "CAP_GOODS",
    "JWL": "CAP_GOODS",
    "JYOTICNC": "CAP_GOODS",
    "KEI": "CAP_GOODS",
    "PGEL": "CAP_GOODS",
    "PTCIL": "CAP_GOODS",
    "RRKABEL": "CAP_GOODS",
    "SCHNEIDER": "CAP_GOODS",
    "ENRIN": "CAP_GOODS",
    "TECHNOE": "CAP_GOODS",
    "TEGA": "CAP_GOODS",
    "TARIL": "CAP_GOODS",
    "HONAUT": "CAP_GOODS",
    "3MINDIA": "CAP_GOODS",
    "INOXINDIA": "CAP_GOODS",
    "POWERMECH": "CAP_GOODS",
    "ISGEC": "CAP_GOODS",
    "HFCL": "CAP_GOODS",
    "TRITURBINE": "CAPITAL_GOODS",
    "TDPOWERSYS": "CAPITAL_GOODS",
    "ELECON": "CAPITAL_GOODS",
    "BAJAJELEC": "CAPITAL_GOODS",
    "HAPPYFORGE": "CAPITAL_GOODS",

    # PIPES & CABLES
    "SUPREMEIND": "PIPES_CABLES",
    "FINCABLES": "PIPES_CABLES",
    "FINPIPE": "PIPES_CABLES",
    "APOLLOPIPE": "PIPES_CABLES",
    "PRINCEPIPE": "PIPES_CABLES",

    # DEFENCE & SHIPBUILDING
    "BEL": "DEFENCE",
    "HAL": "DEFENCE",
    "MAZDOCK": "DEFENCE",
    "COCHINSHIP": "DEFENCE",
    "GRSE": "DEFENCE",
    "DATAPATTNS": "DEFENCE",
    "ZENTEC": "DEFENCE",
    "BDL": "DEFENCE",
    "SOLARINDS": "DEFENCE",

    # INFRASTRUCTURE & PSU CONSTRUCTION
    "CONCOR": "INFRA",
    "IRFC": "INFRA",
    "RVNL": "INFRA",
    "IRCON": "INFRA",
    "NBCC": "INFRA",
    "RAILTEL": "INFRA",
    "HUDCO": "INFRA",
    "PNCINFRA": "INFRA",
    "GMRAIRPORT": "INFRA",
    "IRB": "INFRA",
    "ASHOKA": "INFRA",
    "JKIL": "INFRA",
    "AFCONS": "INFRA",
    "CEMPRO": "INFRA",
    "ENGINERSIN": "INFRA",
    "GMRAIRPORT": "INFRA",
    "KPIL": "INFRA",
    "NCC": "INFRA",
    "RITES": "INFRA",
    "PSPPROJECT": "INFRA",
    "HGINFRA": "INFRA",

    # CHEMICALS & SPECIALTY
    "PIDILITIND": "CHEMICALS",
    "ASTRAL": "CHEMICALS",
    "PIIND": "CHEMICALS",
    "SRF": "CHEMICALS",
    "DEEPAKNI": "CHEMICALS",
    "GNFC": "CHEMICALS",
    "TATACHEM": "CHEMICALS",
    "AARTIIND": "CHEMICALS",
    "NAVINFLUOR": "CHEMICALS",
    "ATUL": "CHEMICALS",
    "FINEORG": "CHEMICALS",
    "ALKYLAMINE": "CHEMICALS",
    "PCBL": "CHEMICALS",
    "GALAXYSURF": "CHEMICALS",
    "LAXMICHEM": "CHEMICALS",
    "CLEAN": "CHEMICALS",
    "LINDEINDIA": "CHEMICALS",
    "VINATI": "CHEMICALS",
    "NOCIL": "CHEMICALS",
    "ROSSARI": "CHEMICALS",
    "SUDARSCHEM": "CHEMICALS",
    "DCMSHRIRAM": "CHEMICALS",
    "ACUTAAS": "CHEMICALS",
    "ANURAS": "CHEMICALS",
    "FLUOROCHEM": "CHEMICALS",
    "GFLLIMITED": "CHEMICALS",
    "HSCL": "CHEMICALS",
    "JUBLINGREA": "CHEMICALS",
    "SPLPETRO": "CHEMICALS",
    "BALAMINES": "CHEMICALS",
    "BASF": "CHEMICALS",
    "TATVACC": "CHEMICALS",
    "TATVA": "CHEMICALS",
    "TARSONS": "CHEMICALS",

    # AGRI & FERTILISERS
    "COROMANDEL": "AGRI",
    "RALLIS": "AGRI",
    "SUMICHEM": "AGRI",
    "GHCL": "AGRI",
    "DEEPAKFERT": "AGRI",
    "EIDPARRY": "AGRI",
    "BAYER": "AGRI",
    "GODREJAGRO": "AGRI",
    "UPL": "AGRI",
    "CHAMBLFERT": "AGRI",
    "FACT": "AGRI",
    "PARADEEP": "AGRI",
    "KRBL": "AGRI",
    "LTFOODS": "AGRI",
    "BALRAMCHIN": "AGRI",
    "TRIVENI": "AGRI",
    "KSCL": "AGRI",
    "DHANUKA": "AGRI",

    # PAINTS
    "ASIANPAINT": "PAINTS",
    "BERGEPAINT": "PAINTS",
    "KANSAINER": "PAINTS",
    "AKZOINDIA": "PAINTS",

    # REAL ESTATE
    "DLF": "REAL_ESTATE",
    "GODREJPROP": "REAL_ESTATE",
    "OBEROIRLTY": "REAL_ESTATE",
    "PRESTIGE": "REAL_ESTATE",
    "BRIGADE": "REAL_ESTATE",
    "PHOENIXLTD": "REAL_ESTATE",
    "SOBHA": "REAL_ESTATE",
    "MAHLIFE": "REAL_ESTATE",
    "ANANTRAJ": "REAL_ESTATE",
    "KOLTEPATIL": "REAL_ESTATE",
    "SIGNATURE": "REAL_ESTATE",
    "JSWINFRA": "REAL_ESTATE",
    "ABREL": "REAL_ESTATE",
    "LODHA": "REAL_ESTATE",
    "SUNTECK": "REAL_ESTATE",
    "PURVA": "REAL_ESTATE",

    # TELECOM
    "BHARTIARTL": "TELECOM",
    "INDUSTOWER": "TELECOM",
    "TATACOMM": "TELECOM",
    "BHARTIHEXA": "TELECOM",
    "ITI": "TELECOM",
    "TTML": "TELECOM",
    "TEJASNET": "TELECOM",
    "IDEA": "TELECOM",

    # LOGISTICS & TRANSPORT
    "IRCTC": "LOGISTICS",
    "DELHIVERY": "LOGISTICS",
    "ALLCARGO": "LOGISTICS",
    "BLUEDART": "LOGISTICS",
    "VRL": "LOGISTICS",
    "TCI": "LOGISTICS",
    "MAHLOG": "LOGISTICS",
    "TCIEXP": "LOGISTICS",
    "AEGISLOG": "LOGISTICS",
    "AEGISVOPAK": "LOGISTICS",
    "GESHIP": "LOGISTICS",
    "INDIGO": "LOGISTICS",
    "SCI": "LOGISTICS",
    "GDLLEAS": "LOGISTICS",
    "SNOWMAN": "LOGISTICS",

    # RETAIL & CONSUMER DISCRETIONARY
    "TITAN": "RETAIL",
    "TRENT": "RETAIL",
    "DMART": "RETAIL",
    "NYKAA": "RETAIL",
    "ZOMATO": "RETAIL",
    "PAYTM": "RETAIL",
    "POLICYBZR": "RETAIL",
    "KALYANKJIL": "RETAIL",
    "SENCO": "RETAIL",
    "DOMS": "RETAIL",
    "EASEMYTRIP": "RETAIL",
    "WONDERLA": "RETAIL",
    "UFBL": "RETAIL",
    "MANYAVAR": "RETAIL",
    "OLECTRA": "RETAIL",
    "METRO": "RETAIL",
    "SAPPHIRE": "RETAIL",
    "JUBLFOOD": "RETAIL",
    "DEVYANI": "RETAIL",
    "ABFRL": "RETAIL",
    "ABLBL": "RETAIL",
    "FIRSTCRY": "RETAIL",
    "LENSKART": "RETAIL",
    "MEESHO": "RETAIL",
    "SWIGGY": "RETAIL",
    "VMM": "RETAIL",
    "VAIBHAVGBL": "CONSUMER",

    # TEXTILES
    "PAGEIND": "TEXTILES",
    "RAYMOND": "TEXTILES",
    "ARVIND": "TEXTILES",
    "TRIDENT": "TEXTILES",
    "KPRMILL": "TEXTILES",
    "CANTABIL": "TEXTILES",
    "VTL": "TEXTILES",
    "WELSPUNLIV": "TEXTILES",
    "GOKEX": "TEXTILES",
    "RUPA": "TEXTILES",
    "DOLLAR": "TEXTILES",

    # MEDIA & ENTERTAINMENT
    "PVRINOX": "MEDIA",
    "ZEEL": "MEDIA",
    "SUNTV": "MEDIA",
    "NAZARA": "MEDIA",
    "SAREGAMA": "MEDIA",
    "NETWORK18": "MEDIA",

    # HOSPITALITY
    "INDHOTEL": "HOSPITALITY",
    "LEMONTREE": "HOSPITALITY",
    "CHALET": "HOSPITALITY",
    "JUNIPER": "HOSPITALITY",
    "EIHOTEL": "HOSPITALITY",
    "ITCHOTELS": "HOSPITALITY",
    "THELEELA": "HOSPITALITY",
    "TBOTEK": "HOSPITALITY",
    "TRAVELFOOD": "HOSPITALITY",

    # CONGLOMERATES
    "ADANIENT": "CONGLOMERATES",
    "ADANIPORTS": "CONGLOMERATES",
    "GODREJIND": "CONGLOMERATES",
    "BAJAJHLDNG": "CONGLOMERATES",
    "SWANCORP": "CONGLOMERATES",
    "JSWHL": "CONGLOMERATES",
    "TATAINVEST": "CONGLOMERATES",

    # OTHER
    "ARE&M": "OTHER",
    "BAYERCROP": "OTHER",
    "BLS": "OTHER",
    "CANBK": "OTHER",
    "CONCORDBIO": "OTHER",
    "CUMMINSIND": "OTHER",
    "DEEPAKNTR": "OTHER",
    "ETERNAL": "OTHER",
    "HEXT": "OTHER",
    "IIFL": "OTHER",
    "INDIAMART": "OTHER",
    "JUSTDIAL": "OTHER",
    "KALPATARU": "OTHER",
    "KAYNES": "OTHER",
    "LXCHEM": "OTHER",
    "METROBRAND": "OTHER",
    "MOTILALOFS": "OTHER",
    "MTARTECH": "OTHER",
    "NAM-INDIA": "OTHER",
    "POLYPLEX": "OTHER",
    "PWL": "OTHER",
    "RAJESHEXPO": "OTHER",
    "URBANCO": "OTHER",
    "VINATIORGA": "OTHER",
    "VRLLOG": "OTHER",
    "ZENSARTECH": "OTHER",
}